...
class UserProductMvcTest {
    ...
    @BeforeEach
    public void setup() {
        mvc = MockMvcBuilders.webAppContextSetup(context)      // MockMvc를 이용해서 Http 요청을 가짜로 날릴 수 있음
                .apply(springSecurity(new MockSpringSecurityFilter()))   //아까 excludeFilters로 제외시켜 놓았으니, springSecurity에 우리가 만들어준 filter를 넣어줌 -> apply에 파라미터로 전달.
                .build();
    }
    ...
    void test1() throws Exception {
        ...
        mvc.perform(get("/api/user/login-page"))
                ...
                .andExpect(view().name("login"))  //view라는 메서드도 있다. -> 반환되는 html 파일 이름 예측
                .andDo(print());
    }
    ...
    void test2() throws Exception {
        // given
        MultiValueMap<String, String> signupRequestForm = new LinkedMultiValueMap<>(); // MultiValueMap: Json 형식의 Body -> 요청 들어오는 데이터를 모방해준것
        ...
        // when - then
        mvc.perform(post("/api/user/signup")      //이렇게해야 JwtAuthenticationFilter를 탄다.
                        .params(signupRequestForm)
                )
                .andExpect(status().is3xxRedirection())  //redirect 하면 300대 코드니까 3xx
    ...
    void test3() throws Exception {
        // given
        this.mockUserSetup();  //아까 만들어둔 메서드 한번 호출.
        ...
        String postInfo = objectMapper.writeValueAsString(requestDto);  //java 객체를 JSON 문자열로 변환 (서버에서는 Json형태의 객체를 String으로 바꿔서 보내기 때문에, 그것을 모방 한 것.)
